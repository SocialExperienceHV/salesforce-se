...
export default function PlanTrabajoPage() {
  ...
  function handleNuevaLabor() {
    ...
    // Esta persona ya tenía esta labor y ya está Finalizada: no reabrirla en silencio
    if (override?.estado === 'Finalizado') {
      setNuevaError(`${nuevaPersona} ya tiene "${proyecto.nombre}" asignada y finalizada. Reprográmala desde su tarjeta si hay que retomarla.`)
      return
    }
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div>
            {!nuevaLaborOpen ? (
              ...
              <div style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 12, padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <span style={{ fontSize: 10, fontWeight: 600, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Proyecto</span>
                    <select value={nuevaProyecto} onChange={e => { setNuevaProyecto(e.target.value); setNuevaError('') }} style={{ ...sel, minWidth: 200 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <span style={{ fontSize: 10, fontWeight: 600, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Persona</span>
                    <select value={nuevaPersona} onChange={e => { setNuevaPersona(e.target.value); setNuevaError('') }} style={{ ...sel, minWidth: 160 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    ...
                    <select value={nuevaDia} onChange={e => { setNuevaDia(e.target.value as DiaPlan); setNuevaError('') }} style={{ ...sel, minWidth: 130 }}>
                  ...
                  <button onClick={() => { setNuevaLaborOpen(false); setNuevaProyecto(''); setNuevaPersona(''); setNuevaDia('Lunes'); setNuevaError('') }}
                ...
                {nuevaError && <p style={{ fontSize: 12, color: '#DC2626', fontWeight: 500, margin: 0 }}>{nuevaError}</p>}
              </div>
            )}
          </div>
